...
export const patternModeFormQuestionsSchema = z.strictObject({
  ...
  forBottomCoordinateUseBottom: z.array(z.enum(['line', 'vector', 'image'])).optional(),

  forTopCoordinateLookUp: z.strictObject({
    by: z.number().positive(),
    chainBy: z.number().nonnegative().optional(),
    for: z.array(z.enum(['line', 'vector', 'image']))
      .min(1, { message: 'At least one of line, vector, or image is required' })
  ...
  mergeQuestions: z.strictObject({
    splitBy: z.enum(['pages', 'columns']),
    mergeOnlyIfContainsAny: z.array(z.enum(['line', 'vector', 'image'])),
  }).optional(),

  paragraphQuestionsCommonPart: z.strictObject({
    pattern: regexOrTextPatternSchema,
    searchIn: searchAreaSchema,
  }).optional(),

  numOfOptionalQuestions: z.number().nonnegative(),
...
export const subjectFormSchema = subjectWithSectionsSchema.extend({
  sections: z.array(sectionSchema),
})

export type PatternModeSubjectWithSectionsConfigJson = z.infer<typeof subjectWithSectionsSchema>
export type PatternModeSubjectWithoutSectionsConfigJson = z.infer<typeof subjectWithoutSectionsSchema>
...
// console.log(JSON.stringify(z.toJSONSchema(patternModeConfigSchema, { io: 'input', reused: 'ref' }), null, 2))
