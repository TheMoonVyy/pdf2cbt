...
function prepareInstructionsData() {
  const currentData = testConfig.additionalData
  ...
  for (const [subject, sections] of Object.entries(subjectSections)) {
    ...
    for (const section of sections) {
      ...
      const data = type === 'none'
        ? ''
        : ((currentSectionData?.instructions as { data?: string })?.data || '')
  ...
  if (testConfig.testInstructions?.type) {
    ...
    if (type === 'custom') {
      instructionsData.testInstructions = {
        ...testConfig.testInstructions,
        pages: testConfig.testInstructions.pages
          .map(page => ({ title: page.title, data: page.data, parsedData: '' })),
      }
    }
    else {
      instructionsData.testInstructions.type = type
    }
  }
}
