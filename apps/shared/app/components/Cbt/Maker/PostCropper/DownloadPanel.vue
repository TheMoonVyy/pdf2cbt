...
function transformDataToOutputFormat(
  ...
  for (const [subject, subjectData] of Object.entries(instructionsData.additionalData)) {
    for (const [section, sectionData] of Object.entries(subjectData.sections)) {
      ...
      else if (instructions.type === 'custom') {
        additionalData[subject].sections[section] = {
          optionalQuestions,
          instructions: { type: 'custom', data: instructions.data },
        }
      }
      else {
        additionalData[subject].sections[section] = {
  ...
  if (type === 'custom') {
    testInstructions = {
      type: 'custom',
      pages: instructionsData.testInstructions.pages
        .map(page => ({ title: page.title, data: page.data })),
      declaration: instructionsData.testInstructions.declaration,
    }
  }
...
async function zipAndSaveOutput() {
