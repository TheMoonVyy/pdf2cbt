...
function parsePageRange(includeStr: string, excludeStr: string) {
  ...
  function parsePages(str: string) {
    const parts = str
      .toUpperCase()
      .split(',')
      .map(s => s.trim())
...
const dummyValue = shallowRef(true)
...
<template>
  <div class="flex flex-col w-full grow pb-50">
          ...
          <div class="flex gap-6 border rounded-md divide-x">
            ...
            <div class="flex flex-col items-center gap-1 py-2 px-4">
              <div class="flex items-center justify-center gap-2">
                ...
                <IconWithTooltip
                  :tooltip-content="tooltipContent.searchRegion.pageNums"
                  icon-class="text-xl"
                />
              </div>
    ...
    <BaseFlexibleDialog
      v-model="dummyValue"
      title="Text Pattern Mode Docs"
    >
      <DocsPdfCropper class="px-4" />
      <!-- Hello -->
    </BaseFlexibleDialog>
